...
<!-- SPEAKER note: mention that all async functions/blocks automatically implement the future trait, so from now on when we refer to future we mean async operation or block -->
...
- Initialize and create a MQTT client and connect to the broker
